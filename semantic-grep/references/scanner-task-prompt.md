...
Use this prompt template when spawning scanner tasks in Step 4.
...
- Always use --metrics=off (prevents sending telemetry)
- Use --pro when Pro is available (enables cross-file taint tracking)
- Run all rulesets in parallel with & and wait
- For GitHub URLs, pass the URL directly to semgrep (it handles them natively)
...
## Critical Rules
- Use ONLY the rulesets listed above
