 ...
 */
async function analyzeSinglePath(
  projectPath: string,
  ...
  const [packageJson, cargoToml, pyProjectToml, envVars, configFiles, existingSkills] = await Promise.all([
    ...
    options.skipEnvFiles ? Promise.resolve({}) : readEnvVars(projectPath),
    findConfigFiles(projectPath),
    options.skipSkillDetection ? Promise.resolve([]) : findExistingSkills(projectPath)
  ]);
...
function mergeAnalysisResults(results: ProjectAnalysis[], rootPath: string, workspaces: string[]): ProjectAnalysis {
  ...
  function addTech(arr: DetectedTechnology[], tech: DetectedTechnology) {
 ...
 * @returns Complete project analysis with detected technologies
 *
 * @example
 * ```typescript
...
export async function analyzeProject(
  ...
  const workspaces = options.skipWorkspaces ? [] : await findWorkspaces(projectPath);
  ...
  const allResults = await Promise.all(pathsToAnalyze.map(p => analyzeSinglePath(p, options)));
 ...
 */
export function getAllTechnologies(analysis: ProjectAnalysis): DetectedTechnology[] {
  return [
    ...analysis.languages,
    ...analysis.frameworks,
    ...analysis.deployment,
...
/**
 * Create a detection context for testing or custom detection
 */
export async function createDetectionContext(
  projectPath: string,
  options: AnalysisOptions = {}
): Promise<DetectionContext> {
  const [packageJson, cargoToml, pyProjectToml, envVars, configFiles] = await Promise.all([
    readPackageJson(projectPath),
    readCargoToml(projectPath),
    readPyProjectToml(projectPath),
    options.skipEnvFiles ? Promise.resolve({}) : readEnvVars(projectPath),
