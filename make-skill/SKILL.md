...
```yaml
category: testing|development|documentation|refactoring|security|performance|code-quality|deployment|database|framework|workflow|memory|communication|devops
user-invocable: true
...
## Local Path Workflow (Before Publish)

When package versions in npm are behind local repo changes, use local paths:

```bash
# From hooks repo, rebuild local packages after schema/category changes
cd ../hooks
npm run build -w @4meta5/skill-loader
npm run build -w @4meta5/skills
npm run build -w @4meta5/skills-cli

# From skills repo, install into target project using local hooks CLI
cd ../skills
node ../hooks/packages/cli/bin/skills.js add <skill-name> --cwd /absolute/path/to/project
```

This avoids waiting for published package updates.

## Adding A New Category (Cross-Repo Contract)

If introducing a new category (example: `devops`), update all category contracts together:

1. `../hooks/packages/skill-loader/src/types.ts` (parser validation source of truth)
2. `../hooks/packages/skills/src/types.ts` (skills library type contract)
3. `../hooks/packages/cli/src/detector/types.ts` (CLI recommendation category union)
4. Relevant docs that enumerate categories:
   - `../hooks/packages/skills/README.md`
   - `../hooks/packages/skill-loader/README.md`
   - `../hooks/packages/cli/README.md`
5. Rebuild hooks packages locally before testing/installing (see Local Path Workflow).

Rules:
- Use lowercase category keys in frontmatter (e.g. `devops`, not `Devops`).
- Do not add a new category in a skill until validation contracts above are updated.
