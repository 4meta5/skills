...
- **Valid Values**:
  ...
  - `code-quality` - Linting/review quality workflows
  - `deployment` - Deployment/runtime operations
  - `database` - Database-focused workflows
  - `framework` - Framework-specific workflows
  - `workflow` - Process orchestration workflows
  - `memory` - Memory/state management workflows
  - `communication` - Human/team communication workflows
  - `devops` - Only valid after cross-repo category contract is updated
...
## Adding A New Category (Required Cross-Repo Updates)

Before using a new category in a SKILL.md, update all of:

1. `../hooks/packages/skill-loader/src/types.ts`
2. `../hooks/packages/skills/src/types.ts`
3. `../hooks/packages/cli/src/detector/types.ts`
4. Category lists in package READMEs

Then rebuild local hooks packages and run `skills validate`.

Use lowercase category values (example: `devops`).
